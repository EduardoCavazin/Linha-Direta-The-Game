...
class NameInputScreen:
    def __init__(self, screen: pygame.Surface, game_time: int):
        ...
        background_path = os.path.join(project_root, 'assets', 'ui', 'menu', 'newrecord.png')
        
        try:
            self.background = pygame.image.load(background_path)
            self.background = pygame.transform.scale(self.background, (screen.get_width(), screen.get_height()))
        except (FileNotFoundError, pygame.error) as e:
            log_error(f"Erro ao carregar background do new record", "nameInputScreen", e)
            self.background = None
        
        self.background_color = (20, 30, 40)  
        self.text_color = (255, 255, 255)     
        self.input_color = (60, 80, 100)      
        self.input_active_color = (80, 120, 160) 
        self.green_accent = (50, 200, 50)    
